...
def registrar_bloques(request):
    ...
    if request.method == 'POST':
        ...
        if not MaePonencia.objects.filter(idcongreso=int(idcongreso)).exists() or not MaePonencia.objects.filter(estado='ACTIVO').exists():
            messages.warning(request,'No hay ponencias registradas o activas, por favor registre al menos una')
        elif not MaeDia.objects.filter(idcongreso=int(idcongreso)).exists() or not MaeDia.objects.filter(estado='ACTIVO').exists():
        ...
        bloques = MaeBloque.objects.filter(idcongreso=int(idcongreso), estado='ACTIVO')
        lista_ponencias = MaePonencia.objects.filter(idcongreso=int(idcongreso), estado='ACTIVO').order_by('pk')
        lista_dias = MaeDia.objects.filter(idcongreso=int(idcongreso), estado='ACTIVO').order_by('pk')
        ...
        if not MaeCongresoJinis.objects.filter().exists() or not MaeCongresoJinis.objects.filter(estado='ACTIVO').exists():
